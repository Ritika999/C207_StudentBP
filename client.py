...
def leftBoard():
    ...
    for box in range(0,11):
        if(box == 0):
            ...
            boxLabel = Label(gameWindow, font=("Helvetica",35), width=2, height=-1, relief='ridge', borderwidth=0, bg="white")
...
def rightBoard():
    ...
    for box in range(0,11):
        if(box == 10):
            boxLabel = Label(gameWindow, font=("Helvetica",20), width=2, height=1, relief='ridge', borderwidth=0, bg="yellow")
            boxLabel.place(x=xPos, y=screen_height/2-88)
...
def finishingBox():
    ...
    finishingBox = Label(gameWindow, text="Home", font=("Chalkboard SE", 22), width=8, height=4, borderwidth=0, bg="green", fg="white")
    finishingBox.place(x=screen_width/2 - 70, y=screen_height/2 -140)
...
def gameWindow():
    ...
    bgimg = ImageTk.PhotoImage(img.resize((1600, 880), Image.ANTIALIAS))     #  this line is for resizing the image to fit canvas size

    canvas2 = Canvas( gameWindow, width = 40,height = 40)
    canvas2.pack(fill = "both", expand = True)

    # Display image
    canvas2.create_image( 0, 0, image = bgimg, anchor = "nw")
    ...
    winingMessage = canvas2.create_text(screen_width/2 + 10, screen_height/2 + 280, text = "", font=("Chalkboard SE",30), fill='#fff176')
...
def askPlayerName():
    ...
    canvas1 = Canvas( nameWindow, width = 40,height = 40)
...
def recivedMsg():
    ...
    while True:
        ...
        if('player_type' in message):
            recvMsg = eval(message)     #eval() converts message into dict format
        ...
        # if message contains 'wins the game' then we call the handleWin() 
        # function tht will help to hide roll button and show reset button
